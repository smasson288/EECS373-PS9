...
int main(int argc, char** argv) {
	...
	ros::ServiceClient startClient = nh.serviceClient<std_srvs::Trigger>("/ariac/start_competition");
	std_srvs::Trigger start_trigger;

	//starting competition
    cout<<"enter 1 to start competition";
    cin>>ans;
	startClient.call(start_trigger);
	...
	//remove all orphan parts from box
	while(orphan_models_wrt_world.size() > 0){
		...
		ROS_INFO("part removed from box");
		//re-inspect box
		boxInspector.update_inspection(desired_models_wrt_world,
		    satisfied_models_wrt_world,misplaced_models_actual_coords_wrt_world,
		    misplaced_models_desired_coords_wrt_world,missing_models_wrt_world,
...
//    cout << "enter 1 to re-inspect: "; //poor-man's breakpoint
	...
	while(nparts_misplaced > 0){
		...
		//re-update inspection once part is moved
	    boxInspector.update_inspection(desired_models_wrt_world,
            satisfied_models_wrt_world, misplaced_models_actual_coords_wrt_world,
            misplaced_models_desired_coords_wrt_world, missing_models_wrt_world,
            orphan_models_wrt_world, part_indices_missing, part_indices_misplaced,
		...
		ROS_INFO("%d misplaced parts left", nparts_misplaced);
	}
...
ROS_INFO("found %d missing parts", (int) part_indices_missing.size());
	while(part_indices_missing.size() > 0) {
        int n_missing_part = part_indices_missing[0];

        //model_to_part(desired_models_wrt_world[n_missing_part], current_part, inventory_msgs::Part::QUALITY_SENSOR_1);
